 <tr class="text-center" id="row{{ $user->id }}">
        ...
        <td>
            <div class="btn-group">
                ...
                <button href="#" class="btn btn-secondary btn-sm open-modal" title="{{ __('pages/orders.edit')}}" value="{{$user->id}}"> <span class="fa fa-pencil"></span> </button>
                @endcan
                @can(\App\Policies\Ability::DELETE, \App\Models\User::class)
                <button href="#" class="btn btn-danger btn-sm delete-user" title="{{ __('pages/orders.delete')}}" value="{{$user->id}}"> <span class="fa fa-trash"></span> </button>
                @endcan
            </div>
        </td>
    </tr>
